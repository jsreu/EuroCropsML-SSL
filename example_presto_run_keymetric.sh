...
KEY_METRIC="multiclassAcc_macro"
  ...
  do
    ...
    do
      ...
      do        
        ...
        eurocropsssl-cli experiments eurocrops ${PHASE} eurocrops_finetuning_maxsamples_${MAX_SAMPLES} base_name=${BASENAME} model=${MODEL} +eurocrops_dataset.data_sources=${DATASOURCE_LIST} finetune=${LR_SETTING} finetune.key_metric=${KEY_METRIC} finetune.train_config.batch_size=${BATCH_SIZE} --run-name=${NAME} --pretrain-run=${ALGORITHMNAME}
      done
    done
  done
done
